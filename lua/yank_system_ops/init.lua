...
local function __create_zip(items, base_dir, filetype)
    ...
    -- Build relative paths for 7z (skip . and ..)
    local rel_items = {}
    for _, f in ipairs(items) do
...
-- The function downloads the file once to a temporary location, detects
-- the proper extension using Content-Type headers, content sniffing, and
-- magic numbers, then copies it to the target directory with the correct name.
--
-- @param uri string URL or FTP link to download
